...
func main() {
	...
	for _, intersection := range intersections {
		currentSecondResult := gridOne[intersection] + gridTwo[intersection]
		if secondResult == 0  || currentSecondResult < secondResult {
			secondResult = currentSecondResult
		}
	}
	fmt.Printf("second: %v\n", secondResult)
}

func getGrid(input string) (map[Point]int, error) {
	var currentX int
	var currentY int
	var steps int
...
func getMoves(input string) ([]Move, error) {
	...
	allMoves := make([]Move, 0, len(moves))
	for _, v := range moves {
		...
		if err != nil {
			return nil, fmt.Errorf("distance to number: %w", err)
		}
...
func getIntersections(gridA map[Point]int, gridB map[Point]int) []Point {
	var intersections []Point

	for point := range gridA {
		if _, exists := gridB[point]; exists {
...
func calculateManhattan(x int, y int) int {
